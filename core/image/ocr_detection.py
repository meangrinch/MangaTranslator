...
class OutsideTextDetector:
    ...
    def detect_outside_text(
        ...
        Args:
            ...
            text_free_only: When True, only target text_free detections from the RT-DETR model
                (narration-style text blocks) and ignore the YOLO OSBText model.
        ...
        if provided_bubble_boxes:
            ...
            # text_free_only still needs text_free boxes; if none were collected
            # upstream, the secondary detection below must run even though YOLO was
            # skipped.
        else:
        ...
        # Secondary RT-DETR: run when text_free_only needs text_free boxes (none were
        # collected upstream) or when YOLO ran here (provided bubbles already include
        # RT-DETR output).
        if (text_free_only and not text_free_boxes) or not provided_bubble_boxes:
            log_message(
        ...
        if text_free_only:
            log_message(
                "Using RT-DETR text_free detections as OSB regions "
                "(skipping YOLO OSB model)",
                always_print=True,
            )
            if text_free_boxes:
                log_message(
                    f"Using {len(text_free_boxes)} text_free boxes as OSB detections",
                    always_print=True,
                )
                ...
                log_message(
                    "No text_free detections available; skipping OSB text detections",
                    always_print=True,
                )
        else:
            log_message("Running YOLO OSB Text...", always_print=True)
            try:
                osbtext_model_path = str(
                    self.manager.model_paths[ModelType.YOLO_OSBTEXT]
                )
                ...
                if cached_osbtext is not None:
                    ...
                    self.cache.set_yolo_detection(
                        osbtext_cache_key,
                        (osbtext_results, osbtext_boxes, osbtext_confs),
                    )
            except Exception as e:
                log_message(

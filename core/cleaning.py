    ...
    try:
        ...
        for detection in detections:
            ...
            try:
                ...
                if len(points.shape) == 3 and points.shape[1] == 1:
                    ...
                    log_message(f"Unexpected mask points format for detection {detection.get('bbox')}. Skipping.", verbose=verbose)
                    continue
                ...
                if masked_pixels.size == 0:
                    log_message(f"Skipping detection {detection.get('bbox')} due to empty mask after indexing.", verbose=verbose)
                    continue
                ...
                log_message(f"Detection {detection.get('bbox')}: Mean={mean_pixel_value:.1f} -> {'Black' if is_black_bubble else 'White'} Bubble. Fill: {fill_color_bgr}", verbose=verbose)
                ...
                if use_otsu_threshold:
                    if np.any(roi_indices):
                        ...
                        if roi_pixels_for_otsu.size > 0:
                            thresh_val, _ = cv2.threshold(roi_pixels_for_otsu, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
                            ...
                            log_message(f"  - Skipping Otsu: No pixels in ROI for thresholding.", verbose=verbose)
                    else:
                        log_message(f"  - Skipping Otsu: ROI mask is empty.", verbose=verbose)
                else:
                    fixed_threshold = 210
                ...
                if is_black_bubble:
                    log_message(f"Detection {detection.get('bbox')}: Applying BLACK bubble refinement logic.", verbose=verbose)
                    ...
                    if contours:
                        ...
                        if valid_contours:
                            ...
                            cv2.drawContours(refined_background_shape_mask, [largest_contour], -1, 255, thickness=cv2.FILLED)
                            log_message(f"  - Found refined background shape.", verbose=verbose)
                        else:
                            log_message(f"  - No valid contours found for refined shape.", verbose=verbose)
                    else:
                        log_message(f"  - No contours found at all for refined shape.", verbose=verbose)

                    base_mask = original_yolo_mask.copy()
                    refined_mask = None
                    if refined_background_shape_mask is not None:
                        dilate_iter = 1
                        dilated_shape_mask = cv2.dilate(refined_background_shape_mask, closing_kernel, iterations=dilate_iter)
                        refined_mask = cv2.bitwise_and(base_mask, dilated_shape_mask)
                        log_message(f"  - Refined mask using dilated shape intersection.", verbose=verbose)
                    else:
                        refined_mask = cv2.morphologyEx(base_mask, cv2.MORPH_CLOSE, closing_kernel, iterations=1)
                        log_message(f"  - No refined shape, using closed base mask (1 iter).", verbose=verbose)

                    # Step 5: Apply Closing and Final Constraints
                    # --- Apply Erosion FIRST to reduce protrusions within the refined mask ---
                    # Use stronger erosion here specifically for black bubble internal protrusion removal
                    internal_erosion_iterations = max(1, (constraint_erosion_iterations + 2)) # 2 additional iterations, ensure at least 1
                    eroded_refined_mask = cv2.erode(refined_mask, constraint_erosion_kernel, iterations=internal_erosion_iterations)
                    # --- Close to smooth and fill holes ---
                    closed_eroded_refined_mask = cv2.morphologyEx(eroded_refined_mask, cv2.MORPH_CLOSE, closing_kernel, iterations=closing_iterations) # Use configured iterations here
                    # --- Apply the outer boundary constraint ---
                    final_mask = cv2.bitwise_and(closed_eroded_refined_mask, eroded_constraint_mask)

                else:
                    log_message(f"Detection {detection.get('bbox')}: Applying WHITE bubble refinement logic (original).", verbose=verbose)
                    contours, _ = cv2.findContours(thresholded_roi, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    if contours:
                        valid_contours = [cnt for cnt in contours if cv2.contourArea(cnt) > min_contour_area]
                        if valid_contours:
                            ...
                            log_message(f"  - Generated mask from largest contour, closed, and constrained.", verbose=verbose)
                        else:
                            log_message(f"  - No valid contours found.", verbose=verbose)
                    else:
        ...
        for bubble_info in processed_bubbles:
            ...
            cleaned_image = np.where(
                np.expand_dims(mask, axis=2) == 255,
                fill_color_image,
                cleaned_image
            )

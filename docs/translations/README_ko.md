...
AI를 사용하여 만화/코믹스 페이지 이미지 번역을 자동화하는 Gradio 기반 웹 애플리케이션입니다. 말풍선 내부 텍스트와 말풍선 외부의 텍스트를 모두 처리합니다. 59개 언어 및 사용자 정의 폰트 팩 사용을 지원합니다.
...
- _Komika_ (일반 텍스트)
- _Cookies_ (말풍선 외부 텍스트)
- _Comicka_ (둘 다 지원)
- _Roboto_ (악센트 기호 지원)
- _Noto Sans SC_ (간체 중국어 지원)
...
4. Nunchaku 설치 (선택 사항, Flux.1 Kontext Nunchaku 백엔드용)

- Nunchaku의 wheel 파일은 PyPI에 등록되어 있지 않습니다. OS 및 Python 버전에 맞는 파일을 v1.2.1 GitHub 릴리스 URL에서 직접 설치하세요. CUDA 전용이며, RTX 2000 시리즈 이상의 그래픽 카드가 필요합니다.

```bash
# 예시 (Windows, Python 3.13, PyTorch 2.10.0, CUDA 13.0)
...
> [!NOTE]
> SDNQ 백엔드를 통해 Flux 모델을 사용할 경우 Nunchaku는 필수 사항이 아닙니다.
...
- 지원 제공자: Google, OpenAI, Anthropic, xAI, DeepSeek, Z.ai, Moonshot AI, OpenRouter, OpenAI-Compatible (OpenAI 호환)
- 웹 UI: Config (설정) 탭에서 제공자/모델/API 키를 설정합니다 (로컬에 저장됨).
- CLI: 명령줄 플래그 또는 환경 변수를 통해 키/URL을 전달합니다.
- 환경 변수: `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `XAI_API_KEY`, `DEEPSEEK_API_KEY`, `ZAI_API_KEY`, `MOONSHOT_API_KEY`, `OPENROUTER_API_KEY`, `OPENAI_COMPATIBLE_API_KEY`
...
2. 다음 주소로 이동하여 이용약관에 동의합니다:
   ...
   - [FLUX.1 Kontext (dev)](https://huggingface.co/black-forest-labs/FLUX.1-Kontext-dev) (Nunchaku와 함께 Kontext 모델을 사용하는 경우 선택 사항)
...
- Comic 말풍선 감지기 YOLOv8m: [ogkalu](https://huggingface.co/ogkalu/comic-speech-bubble-detector-yolov8m)
